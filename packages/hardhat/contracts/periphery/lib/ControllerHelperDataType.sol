...
library ControllerHelperDataType {
    ...
    struct SwapExactoutEthWPowerPerpData {
        ...
        uint256 collateralToWithdraw; // BCH amount to withdraw from vault
    }
    ...
    struct CloseShortWithUserNftParams {
        ...
        uint256 wPowerPerpAmountToBurn; // amount of wPowerPerp to burn in vault
        uint256 collateralToWithdraw; // amount of BCH collateral to withdraw from vault
        uint256 limitPriceEthPerPowerPerp; // price limit for swapping between wPowerPerp and BCH (BCH per 1 wPowerPerp)
    ...
    struct MintAndLpParams {
        ...
        address wPowerPerpPool; // Uni v3 BCH/WPowerPerp pool
        uint256 vaultId;    // vault ID
    ...
    struct FlashloanWMintLpDepositNftParams {
        ...
        uint256 wPowerPerpAmount; // wPowerPerp amount to mint
        uint256 collateralToDeposit; // BCH collateral amount to deposit in vault (including the flashloaned amount to use as collateral in vault)
        uint256 collateralToFlashloan; // BCH amount to flashloan and use for deposit into vault
        uint256 collateralToLp; // BCH collateral amount to use for LPing (could be zero)
        uint256 collateralToWithdraw; // BCH amount to withdraw from vault (if collateralToLp>0, this should be = collateralToLp+fee or 50% of collateralToLP and sender include the rest in msg.value)
    ...
    struct FlashloanCloseVaultLpNftParam {
        ...
        uint256 liquidityPercentage; // percentage of liquidity to burn in LP position in decimals with 18 precision(e.g 60% = 0.6 = 6e17)
        uint256 wPowerPerpAmountToBurn; // amount of wPowerPerp to burn in vault
        uint256 collateralToFlashloan; // amount of BCH collateral to flashloan and deposit into vault
        uint256 collateralToWithdraw; // amount of BCH to withdraw
        uint256 limitPriceEthPerPowerPerp; // price limit for swapping between wPowerPerp and BCH (BCH per 1 wPowerPerp)
        uint128 amount0Min; // minimum amount of token0 to get from closing Uni LP
        uint128 amount1Min; // minimum amount of token1 to get from closing Uni LP
        uint24 poolFee;     // uniswap pool fee
    ...
    struct withdrawFromVaultParams {  
        ...
        bool burnExactRemoved; // if true, will burn the ControllerHelper SBCH balance, to a maximum of the vault short amount
    }
    ...
    struct GeneralSwapParams {
        ...
        uint256 limitPrice; // limit price in units of tokenOut per unit of tokenIn. Notice that this is not always a BCH per wPowerPerp limit price, like is used elsewhere
        uint24 poolFee; // Uniswap pool fee (e.g. 3000 = 30bps)
    }
