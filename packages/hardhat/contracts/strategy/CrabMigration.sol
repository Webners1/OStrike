 ...
 * M4: Can only receive BCH from weth, crabv1, or crabv2 contract
 * M5: Can't withdraw more than you own
 * M6: Not enough BCH to repay the loan
...
contract CrabMigration is Ownable {
     ...
     * @param _strategyCap strategy cap in BCH
     */
    ...
    function _flashCallback(
        ...
        if (FLASH_SOURCE(_callSource) == FLASH_SOURCE.BATCH_MIGRATE) {
            ...
            IERC20(wPowerPerp).approve(crabV1, data.v1SBCHToPay);
            CrabStrategy(crabV1).withdraw(data.crabV1ToWithdraw);

            // flash deposit remaining BCH, otherwise refund
            ...
            uint256 SBCHToPay = IERC20(wPowerPerp).balanceOf(address(this));
            IERC20(wPowerPerp).approve(crabV1, SBCHToPay);

            // find crab amount for contract's sqth balance
            // remaining crab can be withdrawn using flash withdraw
            uint256 crabV1ToWithdrawRmul = SBCHToPay.wmul(CrabStrategy(crabV1).totalSupply()).rdiv(v1Short);
     ...
     * @param _ethToFlashDeposit flash deposit amount in crab v2 with excess BCH (if 0 will return to sender)
    ...
    function flashMigrateFromV1toV2(
        ...
        (bool isFlashOnlyMigrate, uint256 ethNeededForV2, uint256 v1SBCHToPay, ) = _flashMigrationDetails(_v1Shares);
     ...
     * @notice used to migrate from crab V1 to crab V2 when CR1 < CR2
     * @param _v1Shares V1 shares to migrate
     * @param _ethToFlashDeposit flash deposit amount in crab v2 with excess BCH (if 0 will returned to sender)
     * @param _ethToBorrow amount to flash loan to deposit in crab v2
     * @param _withdrawMaxEthToPay maximum BCH to pay to buy back the owed wSqueeth debt
    ...
    function flashMigrateAndWithdrawFromV1toV2(
        ...
        (bool isFlashOnlyMigrate, , uint256 v1SBCHToPay, ) = _flashMigrationDetails(_v1Shares);
        ...
        )
    {
        ...
        uint256 v1SBCHToPay = v1TotalShort.wmul(_v1Shares).wdiv(CrabStrategy(crabV1).totalSupply());
        uint256 ethNeededForV2 = v1SBCHToPay.wmul(v2TotalCollateral).rdiv(v2TotalShort).ceil(10**9) / (10**9);
        uint256 ethToGetFromV1 = _v1Shares.wdiv(CrabStrategy(crabV1).totalSupply()).wmul(v1TotalCollateral);

        return (ethNeededForV2 <= ethToGetFromV1, ethNeededForV2, v1SBCHToPay, ethToGetFromV1);
    }
